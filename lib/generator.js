  ...
  return locals.posts.map(function(post){
    ...
    if(imgMatch){
      for(i=0; i<imgMatch.length; i++){
        ...
        replaceStr = replaceStr.replace(imgMatch[i], '<p><amp-img '+ imgSrc +' width="'+ imgWidth +'" height="'+ imgHeight +'" layout="responsive"></amp-img></p>');
    ...
    // youtube (1920 x 1080)
    //------------------------------------
    var youtubeMatch  = replaceStr.match(/\<iframe\swidth="500"\sheight="281"\sid="youtube_iframe"\ssrc\=\"https\:\/\/www\.youtube\.com\/embed\/[0-9a-zA-Z]+\?feature\=oembed\&amp\;enablejsapi\=1\&amp\;origin\=https\:\/\/safe\.txmblr\.com\&amp\;wmode\=opaque\"\sframeborder\=\"0\"\>\<\/iframe\>/g);
    if(youtubeMatch){
