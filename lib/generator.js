  ...
  return locals.posts.map(function(post){
    ...
    if(imgMatch){
      for(i=0; i<imgMatch.length; i++){
        ...
        imgWidth = (imgDataMatch?.length > 1) ? imgDataMatch[1] : '';

        imgDataMatch = imgMatch[i].match(/max\-height\:\s?([0-9]+)px/);
        imgHeight = (imgDataMatch?.length > 1) ? imgDataMatch[1] : '';
        ...
        var imgName = imgSrc.match(/(.*)\/(.*)\.(.*)$/);
        imgName = imgName[2] + String(Math.floor((Math.random()*100) + 1));
        replaceStr = replaceStr.replace(imgMatch[i], '<div id="' + imgName + '" class="minsh-img"><amp-img '+ imgSrc +' width="'+ imgWidth +'" height="'+ imgHeight +'" layout="responsive"></amp-img></div>');

        cssTxt += " #" + imgName + " { max-width: " + imgWidth + "px; }"
      }
    }
